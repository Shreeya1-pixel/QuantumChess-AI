...
export const exportToPGN = (game: Chess, analysisHistory: MoveAnalysis[]): string => {
  ...
  analysisHistory.forEach((analysis, index) => {
    if (index < moves.length) {
      ...
      const comment = `{Classical AI: ${analysis.classical.move} (${analysis.classical.reason}) | Quantum AI: ${analysis.quantum.move} (${analysis.quantum.reason})}`
...
${analysisHistory.map(a => `- ${a.quantum.concept}: ${a.quantum.reason}`).join('\n')}
